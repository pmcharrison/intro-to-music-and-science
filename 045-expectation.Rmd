...
embed_image(
  ...
  width = "70%",
  title = "Minor-key tone profile derived by @krumhansl1982."
)
```
...
```{r}
embed_image(
  image = "expectation/narmour-small-intervals.png",
  width = "100%",
  title = "Example melody built from small intervals (illustrating Narmour's proximity principle).",
)
```
...
```{r}
embed_image(
  image = "expectation/narmour-implication-realisation.png",
  width = "70%",
  title = "Implicative versus realised intervals."
)
```
...
```{r}
embed_image(
  image = "expectation/narmour-registral-return.png",
  width = "70%",
  title = "Illustration of Narmour's registral return principle."
)
```
...
```{r}
embed_image(
  image = "expectation/pitch-images-60.svg",
  width = "100%",
  title = "Simulating a perceptual spectrum for the MIDI note 60, after @Milne2011-iv. The simulation process goes from top to bottom."
)
```
...
```{r}
embed_image(
  image = "expectation/pitch-images-60-64-67.svg",
  width = "100%",
  title = "Simulating a perceptual spectrum for a C major triad, after @Milne2011-iv. The simulation process goes from top to bottom."
)
```
...
```{r}
embed_image(
  image = "expectation/milne--probe-tone-major-chord-0-combined.png",
  width = "100%",
  title = "Perceptual spectra for a C major triad combined with a C probe tone."
)
```
...
```{r}
embed_image(
  image = "expectation/milne--probe-tone-major-chord-1-combined.png",
  width = "100%",
  title = "Perceptual spectra for a C major triad combined with a C# probe tone."
)
```
...
```{r}
embed_image(
  image = "expectation/milne--krumhansler-kessler.png",
  width = "100%",
  title = "Comparison of the Krumhansl-Kessler [-@krumhansl1982] probe-tone data with the predictions of Milne's spectral similarity model [@Milne2011-iv]."
)
```
...
```{r}
embed_image(
  image = "expectation/probability-continuations.png",
  width = "100%",
  title = "Calculating the probability of a melody's continuation through reference to prior observations."
)
```
...
```{r}
embed_image(
  image = "expectation/multiple-viewpoints.png",
  width = "100%",
  title = "Multiple-viewpoint representation of a melody."
)
```
...
```{r}
embed_image(
  image = "expectation/multiple-viewpoint-context-1.png",
  width = "50%",
  title = "Predicting from a 1st-order Markov model."
)
```
...
```{r}
embed_image(
  image = "expectation/multiple-viewpoint-context-2.png",
  width = "50%",
  title = "Predicting from a 2nd-order Markov model."
)
```
...
```{r}
embed_image(
  image = "expectation/multiple-viewpoint-context-3.png",
  width = "50%",
  title = "Predicting from a 3rd-order Markov model."
)
```
...
chorale_image <- magick::image_read_svg(
  "expectation/idyom-dynamic-predictions-1.svg"
)

chorale <- 
  cowplot::ggdraw() +
  cowplot::draw_image(chorale_image)

graph <- 
  read_csv("expectation/idyom-dynamic-predictions.csv", col_types = cols()) |> 
  mutate(
    type = factor(type, levels = c("IDyOM", "Human"))
  ) |> 
  ggplot(aes(note_position, surprise, colour = type)) +
  geom_line() + 
  scale_x_continuous("Note position") + 
  scale_y_continuous("Surprise") +
...
ggsave("expectation/idyom-dynamic-predictions-combined.svg", width = 6, height = 6)

embed_image(
  image = "expectation/idyom-dynamic-predictions-combined.svg",
  width = "100%",
  title = "Modelling listener surprise through the course of a melody [data from @Pearce2018-pw]."
)
```
