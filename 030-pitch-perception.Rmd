...
The pitch notation systems used in Western music reflect this primal role of pitch intervals in music perception.

## Scales

A scale defines a collection of pitches from which a musical passage is created. In Western music we are particularly familiar with the major and minor scales, but many more scales can be identified both within Western and non-Western musical traditions.

Most known scales embody a concept called *octave equivalence*. Octave equivalence means that pitches separated by octaves (i.e. 2:1 frequency relationships) share some kind of underlying identity, which can be termed *chroma* [@Bachem1950-eb]. In Western music theory we use the term *pitch class* to denote collections of pitches separated by octaves. We can label pitch classes with their corresponding letter names (e.g. C, C\#, D, D\#, ...), or alternatively we can label them with numbers, where by convention C is written as 0, C\# is written as 1. A full table can be found below:
...
A scale can then be defined as a collection of pitch classes. For example, the D major scale would be defined by the following pitch classes:

> D, E, F\#, G, A, B, C\#

It is possible to generalise the definition of transposition to include pitch classes. In the context of pitches, transposing meant multiplying

Scales can be transposed

A scale is a collection of notes which...

Most scales embody octave invariance.... chroma/pitch height/shepard ... but see bohlen-pierce scale...

A useful property of a scale is to support transposition. This is used a lot in Western music.

The best way of supporting transposition is if we know that we can take the interval between any two notes in the scale and play that same interval starting on any other note in that scale...

An effective way of achieving this is through equal-tempered scales. An equal-tempered scale is where...

Westerners now use the 12-tone musical scale. The semitone is then ...

## Scale degrees

Pitch intervals play an important role in musical *scales*. A musical scale is a collection of pitches that

The psychological salience of pitch interval has deep consequences for musical scale construction.

The basic unit of the Western chromatic scale is the *semitone*. There are 12 semitones in the octave. If we assume that all these semitones are the same size (this is the *equal-temperament assumption*), then the semitone must correspond to the frequency ratio $1 : 2 ^ \frac{1}{12}$. Here $2 ^ \frac{1}{12}$ is the twelfth root of 2, a number which when multiplied by itself 12 times produces the number 2; expressed in decimals, this is approximately 1.059.

relatively likely to recognise that the two frequencies are related by a 3:2 ratio, corresponding to a perfect fifth.

The notion of *pitch interval* is exceptionally important in Western music, and in most known musical styles across the world. Pitch intervals are the perceptual correlate of frequency ratios. For example, the octave corresponds to a 2:1 frequency ratio; if we take concert A (A4, 440 Hz), and double the frequency (880 Hz), this gives us the A one octave above (A5).
