...
```{r, fig.cap = "Frequency spectrum for a closed C diminished triad rooted on middle C (C4). The chord is modelled as containing 6 harmonics per tone, with amplitudes following a harmonic series. The three tones are differentiated by three different colours.", out.width = "100%"}
expand_harmonics <- function(fundamental_frequencies, n_harmonics = 6) {
...
stopifnot(
  all.equal(optim(0.5, dissonance_curve, control = list(fnscale = -1), 
                  method = "Brent", 
                  lower = 0, 
                  upper = 10)$par, 0.25)
)
...
p <- 
  tibble(
    ...
    dissonance = dissonance_curve(cbw_dist)
  ) %>%
...
$$
1.00 + 1.00 + 1.00 + 0.25 + 0.25 + 0.25 + \ldots = `r s_sq_amp <- sum(spectrum$amplitude ^ 2); round(s_sq_amp, 3)`.
$$
...
To get the final dissonance contribution of the two partials, we then take the previously computed dissonance factor (0.014), multiply it by the product of the amplitudes (0.25), and divide it by the sum of the squared amplitudes (`r round(s_sq_amp, 3)`). This gives us:

$$
\frac{0.014 \times 0.25}{`r round(s_sq_amp, 3)`} = `r (0.014 * 0.25 / s_sq_amp) %>% sprintf("%.5f", .)` 
$$
...
```{r}
get_dissonance_by_partial_pairs <- function(spectrum) {
  n_partials <- nrow(spectrum)
  combinations <- combn(n_partials, 2, simplify = FALSE)
  tibble(
...
dissonance_by_partial_pairs <- get_dissonance_by_partial_pairs(spectrum)
...
```{r, out.width = "100%", fig.cap = "Hutchinson and Knopoff model outputs for various prototypical triads from Western music. Each chord is played in closed form and rooted on middle C."}
list(
  "C major" = c(60, 64, 67),
  "C minor" = c(60, 63, 67),
  ...
  map_dbl(function(midi) {
    freq <- 440 * 2 ^ ((midi - 69) / 12)
    spectrum <- expand_harmonics(freq)
    get_dissonance_by_partial_pairs(spectrum) %>% 
      pull(dissonance) %>% 
      sum()
  }) %>% 
  tibble(
