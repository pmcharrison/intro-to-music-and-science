...
A measurement instrument's *construct* *validity* may be defined as the extent to which it truly measures the underlying trait which it is purported to measure [@Cronbach1955-ep]. It is generally accepted that construct validity can only be established with some reference to theoretical arguments, which are used (a) to define the trait that we are trying to measure and (b) to predict how that trait should manifest in the observable world.

Psychological texts often differentiate various kinds of construct validity. These represent different strategies that a researcher might use to substantiate the construct validity of a given measurement instrument. For example:

-   **Concurrent validity. - gold-standard**
