...
Much of inferential statistics can then be reduced to variants of the following logical process. We suppose that our data are generated by some probability distribution, for example the normal distribution (which happens to resemble many real-world distributions strikingly well). We then try to *infer* the values of this distribution's parameters, for example its mean and standard deviation, based on the data we observe. 
