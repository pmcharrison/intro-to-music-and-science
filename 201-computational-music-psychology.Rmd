...
$$ where $G$ is the [gravitational constant](https://en.wikipedia.org/wiki/Gravitational_constant).
...
> *Brain stem reflex* refers to a process whereby an emotion is induced by music because one or more fundamental acoustic characteristics of the music are taken by the brain stem to signal a potentially important and urgent event that needs attention [\[39\]](https://www.sciencedirect.com/science/article/pii/S1571064513000638#br0390). In music, this could involve sounds that are sudden, loud, or dissonant, or that feature accelerating patterns. Brain stem reflexes are quick, automatic, and unlearned. A famous example that may evoke a brain stem reflex in a listener given a certain sound level is the kettledrum stroke in Joseph Haydnʼs Symphony No. 94 [\[187, p. 81\]](https://www.sciencedirect.com/science/article/pii/S1571064513000638#br1890). More commonly perhaps, such reactions result from dynamic changes in the music (e.g., a full orchestra or band coming in after a short solo intro). Brain stem reflexes may increase *arousal* and evoke feelings of *surprise* in the listener [\[100\]](https://www.sciencedirect.com/science/article/pii/S1571064513000638#br1000).
...
Many of the phenomena we are interested in music psychology (e.g. emotion, expression, aesthetics) are highly complex, relying on many psychological processes that are themselves complex and hard to define. If we did eventually develop a comprehensive and precise theory of musical emotions, we would hardly be able to apply it with a back-of-the-envelope calculation like we could with Newton's theory. However, such calculations become possible once we take advantage of *computational* methods. Computer programs make it possible to construct precise theories that are substantially more complex than Newton's theory, and that require many many calculations for generating predictions. We call this process *computer modelling*.

are the natural extension of equations like Newton's; t

There would be far too many processes involved.

that they could never be summarised by a theory as simple as Newton's law of gravitation.

## Engineering and computer models

Scientific knowledge can be conceptualised as a collection of *theories* about how the world works.

Scientific theories are explanations about how a particular aspect of the world works.
...
```{r critical-bandwidth-frequency, fig.cap = "Critical bandwidth as a function of frequency.", out.width = "100%"}
df_cbw <- 
  tibble(
...
```{r dissonance-curve, fig.cap = "Idealised dissonance curve from @hutchinson1978, plotted using the parametric version specified by @Bigand1996.", animation.hook = "gifski", interval = 0.9, out.width = "100%"}
if (FALSE) {
...
dissonance_by_partial_pairs |> 
  ...
  rename(
    ...
    "Dissonance contribution" = dissonance
  ) |> 
  DT::datatable(
  options = list(
    searching = FALSE,
...
We compute the chord's final dissonance by summing all the 'dissonance contribution' values in the far right column. Many of these values are near-zero, because they relate to partials that are far away from each other. However, if we sum them all together, we get the following value: `r dissonance_by_partial_pairs$dissonance %>% sum() %>% sprintf("%.3f", .)`

Let's now repeat the same approach and analyse several chords.

```{r, out.width = "100%", fig.cap = "Hutchinson & Knopoff model outputs for various prototypical triads from Western music. Each chord is played in closed form and rooted on middle C."}
list(
  "C major" = c(60, 64, 67),
  "C minor" = c(60, 63, 67),
  "C diminished" = c(60, 63, 66),
  "C augmented" = c(60, 64, 68),
  "C sus4" = c(60, 65, 67),
  "C sus2" = c(60, 62, 67)
) %>% 
  map_dbl(dycon::roughness_hutch) %>% 
  tibble(
    label = names(.),
    dissonance = .
  ) %>% 
  ...
  ggplot(aes(label, dissonance, fill = dissonance)) + 
  geom_bar(stat = "identity", colour = "black") + 
  scale_x_discrete(NULL) + 
  scale_y_continuous("Dissonance") + 
