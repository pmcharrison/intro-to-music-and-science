...
The work of researchers like @mehr2019-xu and @Savage2015-rp constitutes important progress towards understanding what musical universals might be out there. A follow-up question, once we have established the existence of such universals, concerns the mechanisms by which such universals arise. Such work is necessarily moderately speculative, but it gets at the root of an essential musicological question: why musical styles developed the way that they did.
...
```{r}
embed_youtube_video("vQhqikWnQCU", width = "100%", title = "Yodelling.", info = "'Auf und auf voll lebenslust', sung by Franzl Lang.")
```
...
We are still learning how musical memory works and how it may relate to the development of musical styles. Nonetheless, we can still set out a few plausible ways in which this might work.
...
```{r}
embed_youtube_video("nAE-5IG0RrU", "An example of a Kentucky accent.", width = "100%")
```

We can contrast this to someone speaking with a Shetland accent. Here the syllables seem much more even.

```{r}
embed_youtube_video("v37bgydws0E", "An example of a Shetland accent.", width = "100%")
```
...
```{r}
embed_youtube_video("Dx49-WxSTnM", "An example of Kentucky fiddle music.", width = "100%")
```
...
```{r}
embed_youtube_video("caFrGTt-drg", "An example of Shetland fiddle music.", width = "100%")
```
...
We've mostly been thinking so far about harmonic tone spectra, because these are the kind of spectra produced by the voice as well as by many other musical instruments. If we put harmonic tones into consonance models and spectral similarity models, we see that consonance and spectral similarity are both maximised when we draw our notes from musical scales containing lots of harmonic intervals, such as the perfect fifth, perfect fourth, and major third. Many musical cultures across the world use scales like this; the Western diatonic scale is just one example.
...
```{r}
embed_youtube_video("qIq8LNbYKT8", width = "100%", title = "Balinese gamelan performance", info = "Ubud Palace, Ubud, Bali, Indonesia, January 2012")
```
...
So, our question is then, can we explain these scale systems in terms of instrument acoustics? We're going to follow an argument by @Sethares2005-ko, which states that these scale systems developed in order to maximise consonance. While we go through this argument, bear in mind that we could also construct an analogous argument that these scale systems developed to maximise spectral similarity, and much of the logic would look the same.

Let's begin with the bonang, one of the key instruments in the gamelan orchestra. The bonang is a collection of small gongs, typically hit with padded sticks.

```{r}
embed_video(video = "videos/bonang.mp4", width = "100%", title = "Demonstration of the bonang", credit = "Feureau, [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/deed.en)")
```
...
```{r}
embed_youtube_video("W1xdd8LpHR4", title = "Demonstration of the saron.", credit = "UK Gamelan Network")
```
